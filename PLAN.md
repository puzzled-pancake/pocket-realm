...
Qualify the single ARM client route—Box64 plus pinned 64-bit Wine WoW64 and an explicitly identified DXVK/Turnip package—by self-test and measured device profile. Keep prefixes and shader caches isolated, bounded, and invalidated by the complete package compatibility ID. FEXCore and client OpenGL are not packaged, selectable, or accepted as fallback. This route remains outside the release critical path until named-device lifecycle and sustained-performance evidence exist.
