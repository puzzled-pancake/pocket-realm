...
The optional Android Port profile (the built-in Android Port add-on) keeps the add-on's normal action-bar and radial-menu keys. L1 on its own chooses the nearest eligible corpse, chest, or ordinary usable object in range and opens it as one action; a lootable corpse opens the normal loot window (auto-loot then works as usual), the realm still applies its normal range, line-of-sight, lock, and loot rules, and nothing appears in chat. The window opening relies on the client-side nearby-loot patch that ships inside every tuned `WoW.exe.patched`; with every client tweak disabled the app launches the pristine client, and L1 then opens loot invisibly. Mashing it about ten times a second can briefly mute chat through the realm's flood filter. R3 left-clicks at the pointer and L3 right-clicks it, so precise manual interaction stays one stick click away; while the camera is locked, L3 jumps instead. Essential leveling controls use Select chords so the unreliable RP6 M1/M2 buttons are not required: hold Select + R1 for stock G / last hostile, Select + L1 for Use / Open at the pointer, Select + R2 for a left mouse click, Select + L3 to jump, Select + Start for Move UI, or Select + R3 for camera/pointer mode. Tap Select by itself for the Android Port radial menu. Opening it unlocks and centres the pointer; move the right stick over an icon and press R3 to open it. Tap Select again to close it.

Move UI (Select + Start, or radial entry 8) puts green drag handles on stock frames and add-on icons. Dragging a handle is the only way a frame ever moves, and a frame may sit flush against the very top or base of the screen. D-pad Down/Up selects the next handle, D-pad Left/Right or the mouse wheel resizes the focused frame around its centre (it does not drift), and Select + Start or Escape saves and exits. Bag and bank frames are scale-only because the stock client re-anchors them when opened. Use `/ap resetui` in chat to restore the untouched stock layout.

Bags (0.6.0) replace the stock bag windows with one all-in-one container while the module is ready; open it from the radial menu's Inventory entry or the usual bag keys. The header's four bag slots are real equipment slots: drag a bag from your inventory onto one to equip it, drag a slot's bag off to lift it, and tap a slot to view that bag alone — All shows every bag at once and Key shows the keyring (holding an item while tapping All or Key tucks it into the backpack or keyring). Sell Junk sells gray quality items only: one item per step, re-checked immediately before every sale, never the keyring, and only while a merchant window is open (tap again to cancel); the earned total prints when it finishes. Vendors and mailboxes that auto-open the backpack land on the container instead of the stock windows, and the container closes again when the vendor closes. A money readout sits in the corner. `/ap bags` toggles the whole module off and back on, restoring the stock behaviour.
...
The touch overlay has Automatic, Controller utilities, Full touch controls, and Off modes. Automatic shows a compact camera/pointer and utility strip when a controller is connected, and the full layout for touch-only play. Full mode comes in two arrangements, both fully rearrangeable: Console (the default) puts a 1-8 action pad on the right, movement with Shift/Ctrl page holds on the left, and look, click, and target keys at the bottom centre; Classic keeps the three pages for actions 1-12 with target, use or loot, and movement clusters. Both include camera drag, camera zoom, and a keyboard button, and every touch button assignment stays editable. Pick the arrangement under Controls, and use Move buttons in the overlay's More menu to drag any cluster wherever it plays best — Reset layout puts them back. On smaller screens the stock arrangement can crowd; Move buttons is the tool for spreading it out comfortably.
...
- L3 toggles auto-run, or jumps while the camera is locked.
