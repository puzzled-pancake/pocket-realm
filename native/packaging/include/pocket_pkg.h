 ...
 *       (useLegacyPackaging=true). The launcher process executes it there. It dlopens the
 *       real libpocketrealm.so by SONAME and reports the dladdr-resolved path.
 *
 *   libpocketpkgtest.so        — a JNI native library used by the smoke driver from
 *       the isolated packaging process. It dlopens libpocketrealm.so by
...
/* Open the real realm shared object BY SONAME (RTLD_NOW), resolve a
...
/* Probe ONE native library by SONAME (RTLD_NOW|RTLD_NOLOAD|RTLD_LOCAL).
 ...
 * 0 on success (info->loaded=1), non-zero otherwise. Used by the smoke probe to prove
...
/* Deterministic crash probe. kind: 0=abort(), 1=NULL-deref, 2=stack-guard. */
