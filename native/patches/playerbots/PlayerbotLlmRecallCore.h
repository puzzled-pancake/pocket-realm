...
// the instruction rides one short closing sentence.
inline std::string DebtCargo(std::string const& playerName, std::string const& factText)
{
    return playerName + " " + FactDirect(factText) +
...
{
    if (askAfter)
        return "You remember what " + playerName + " was after: " +
            FactDirect(factText) + ". Ask after it, like it matters - because it does.";
    return "You DO remember " + playerName + ". One true thing: " +
        FactDirect(factText) + ". Work it in - once, naturally.";
}

inline std::string GrudgeCargo(std::string const& factText)
{
    return "You are still sore about this: " + FactDirect(factText) +
...
{
    return "Something DID happen: " + FactDirect(factText) +
        ". That is your news, with " + playerName + ". Tell it.";
}

inline std::string GossipCargo(std::string const& playerName, std::string const& gossipText)
{
    return "Word is going round about " + playerName + ": '" + gossipText +
...
{
    char const* phrase = "a friend worth keeping";
    if (tier >= 5)
        phrase = "the one you would follow anywhere";
    else if (tier == 4)
        phrase = "a true friend";
    return "You have quietly decided " + playerName + " is " + phrase +
        ". Show it your own way, briefly - and do not explain yourself.";
}

inline std::string CeremonyDownCargo(std::string const& playerName)
{
    return "Something in you has grown colder toward " + playerName +
        ". Do not announce it. Let it change what you do, not what you say.";
}
