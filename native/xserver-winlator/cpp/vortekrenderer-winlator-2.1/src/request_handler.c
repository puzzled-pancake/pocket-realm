...
void vt_handle_vkCreateInstance(VkContext* context) {
    ...
    println("vortek: milestone=CREATE_INSTANCE result=%d", result);
...
void vt_handle_vkEnumeratePhysicalDevices(VkContext* context) {
    ...
    println("vortek: milestone=ENUMERATE_PHYSICAL_DEVICES result=%d count=%u", result, physicalDeviceCount);
...
void vt_handle_vkCreateDevice(VkContext* context) {
    ...
    println("vortek: milestone=CREATE_DEVICE result=%d", result);
...
void vt_handle_vkMapMemory(VkContext* context) {
    ...
    int statResult = -1;
    int statErrno = 0;
    bool shouldLog = vortekReserveMemoryDiagnostic(context);
    if (shouldLog && resourceMemory->fd >= 0) {
        errno = 0;
        statResult = fstat(resourceMemory->fd, &fdStat);
        statErrno = statResult == 0 ? 0 : errno;
    }

    errno = 0;
    int sendResult = send_fds(
        context->clientFd,
        &resourceMemory->fd,
        1,
        &result,
        sizeof(VkResult));
    int sendErrno = sendResult >= 0 ? 0 : errno;
    if (shouldLog) {
        println(
            "vortek-memory: event=send-map route=%d result=%d fd=%d fstat=%d "
            "fstatErrno=%d fdSize=%lld sendmsg=%d sendmsgErrno=%d",
            resourceMemory->transportType,
            result,
            resourceMemory->fd,
            statResult,
            statErrno,
