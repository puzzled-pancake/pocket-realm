...
#define VT_SERIALIZE_CMD(cmdName, ...) \
    ...
    if (bufferSize < 0 || !vt_transport_payload_fits(bufferSize, \
            context->clientRing ? context->clientRing->bufferSize : 0u, \
            CLIENT_RING_BUFFER_SIZE - HEADER_SIZE) || \
            (bufferSize > 0 && !(outputBuffer = \
                    vt_request_output_alloc(&_vt_request, (size_t)bufferSize)))) { \
        vt_request_response_abort(&_vt_request, VT_DECODE_ERROR_LIMIT); \
        return; \
    } \
...
typedef struct VtRequestDecode VtRequestDecode;
...
typedef struct JMethods {
    ...
    JNIEnv* env;
    jobject obj;
    jmethodID registerWindowAuthorityGeneration;
    jmethodID unregisterWindowAuthorityGeneration;
    jmethodID releaseWindowAuthorityInstance;
    jmethodID validateWindowAuthority;
    jmethodID getWindowExtentAuthority;
    jmethodID getWindowHardwareBufferAuthority;
    jmethodID updateWindowContentAuthority;
} JMethods;
...
static inline bool vt_track_pool_allocation(MemoryPool* memoryPool, void* chunk) {
    ArrayList* allocations = memoryPool ? &memoryPool->allocationList : NULL;
    if (!allocations || !chunk || allocations->size < 0 ||
            allocations->capacity < 0 || allocations->size > allocations->capacity ||
            allocations->size == INT_MAX) return false;
    if (allocations->size == allocations->capacity) {
        int newCapacity = allocations->capacity < DEFAULT_ARRAY_CAPACITY
                ? DEFAULT_ARRAY_CAPACITY
                : allocations->capacity + allocations->capacity / 2;
        if (newCapacity <= allocations->capacity) return false;
        if ((size_t)newCapacity > SIZE_MAX / sizeof(*allocations->elements))
            return false;
        void** newElements = realloc(allocations->elements,
                (size_t)newCapacity * sizeof(*allocations->elements));
        if (!newElements) return false;
        memset(newElements + allocations->capacity, 0,
                (size_t)(newCapacity - allocations->capacity) *
                        sizeof(*newElements));
        allocations->elements = newElements;
        allocations->capacity = newCapacity;
    }
    allocations->elements[allocations->size++] = chunk;
    return true;
}

static inline void* vt_alloc(MemoryPool* memoryPool, int size) {
    if (!memoryPool || size <= 0 || memoryPool->size < 0 ||
            memoryPool->size > MEMORY_POOL_MAX_SIZE) return NULL;
    void* chunk = NULL;
    if (memoryPool->data && size <= MEMORY_POOL_MAX_SIZE - memoryPool->size) {
        chunk = (uint8_t*)memoryPool->data + memoryPool->size;
        memoryPool->size += size;
    }
    else {
        chunk = malloc((size_t)size);
        if (!chunk) return NULL;
        if (!vt_track_pool_allocation(memoryPool, chunk)) {
            free(chunk);
            return NULL;
        }
    }

    memset(chunk, 0, (size_t)size);
    return chunk;
}
...
static inline int vt_send(RingBuffer* ring, int requestCode, void* data, int size) {
    if (!ring || size < 0 || (size > 0 && !data)) return 0;
#ifndef VT_SERVER
    ...
    *(int*)(header + 4) = size;
    ...
    if (!vt_transport_payload_fits(size, ring->bufferSize,
            (size_t)ring->bufferSize - HEADER_SIZE)) return 0;
    bool result = RingBuffer_writeFrame(ring, header, HEADER_SIZE,
            data, (uint32_t)size);
    if (!result) return 0;
...
static inline int vt_recv(RingBuffer* ring, char** inputBuffer, int* bufferSize, MemoryPool* memoryPool) {
    if (!ring || !inputBuffer || !memoryPool) return VK_ERROR_DEVICE_LOST;
    *inputBuffer = NULL;
    ...
    int requestCode = 0;
    int size = 0;
    memcpy(&requestCode, header, sizeof(requestCode));
    memcpy(&size, header + sizeof(requestCode), sizeof(size));

    if (!vt_transport_payload_fits(size, ring->bufferSize,
            (size_t)ring->bufferSize - HEADER_SIZE)) {
        return VK_ERROR_DEVICE_LOST;
    }

    if (size > 0) {
        *inputBuffer = vt_alloc(memoryPool, size);
        if (!*inputBuffer) return VK_ERROR_DEVICE_LOST;
