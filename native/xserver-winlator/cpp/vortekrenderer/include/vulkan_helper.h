...
/*
 * Initializes only Pocket Realm's capability-locked first Vortek lane.
 * The caller must destroy `device` and report this result if initialization
 * fails; a successfully-created Vulkan device is not sufficient proof that
 * the bridge's custom paths are safe to use.
 */
extern VkResult initVulkanDevice(VkContext* context, VkPhysicalDevice physicalDevice, VkDevice device);
...
extern void disableUnsupportedDeviceFeatures(
