...
VkResult ShaderInspector_createModule(ShaderInspector* shaderInspector, VkDevice device, const uint32_t* code, size_t codeSize, ShaderModule** ppModule) {
    if (!shaderInspector || !ppModule || !ShaderInspector_validateSpirvEnvelope(code, codeSize)) {
        if (ppModule) *ppModule = NULL;
        return VK_ERROR_INVALID_SHADER_NV;
    }

    *ppModule = NULL;
    ShaderModule* shaderModule = calloc(1, sizeof(ShaderModule));
    if (!shaderModule) return VK_ERROR_OUT_OF_HOST_MEMORY;
    shaderModule->module = VK_NULL_HANDLE;

    if (shaderInspector->passthrough) {
        VkShaderModuleCreateInfo createInfo = {0};
        createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
        createInfo.pCode = code;
        createInfo.codeSize = codeSize;
        VkResult result = vulkanWrapper.vkCreateShaderModule(device, &createInfo, NULL, &shaderModule->module);
...
VkResult ShaderInspector_inspectShaderStages(ShaderInspector* shaderInspector, VkDevice device, VkPipelineShaderStageCreateInfo* stageInfos, uint32_t stageCount, const VkPipelineVertexInputStateCreateInfo* vertexInputState) {
    if (!shaderInspector || (stageCount > 0 && !stageInfos)) return VK_ERROR_VALIDATION_FAILED_EXT;

    if (shaderInspector->passthrough) {
        for (uint32_t i = 0; i < stageCount; i++) {
            ShaderModule* shaderModule = (ShaderModule*)stageInfos[i].module;
            if (!shaderModule || shaderModule->module == VK_NULL_HANDLE) return VK_ERROR_INVALID_SHADER_NV;
            stageInfos[i].module = shaderModule->module;
        }
        return VK_SUCCESS;
    }
