...
void TextureDecoder_destroy(TextureDecoder* textureDecoder) {
    if (!textureDecoder) return;
    while (!ArrayDeque_isEmpty(&textureDecoder->bufferImageCopies)) {
        TextureDecoder_BufferImageCopy* copy =
                ArrayDeque_removeFirst(&textureDecoder->bufferImageCopies);
        free(copy);
    }
    for (int index = textureDecoder->boundBuffers.size - 1; index >= 0; --index)
        free(textureDecoder->boundBuffers.elements[index]);
    /* Authority reclamation destroys every published image before its device.
     * Any remaining node can only name driver state already reclaimed by the
     * device fallback, so release the C ownership without a stale Vulkan call. */
    for (int index = textureDecoder->images.size - 1; index >= 0; --index)
        free(textureDecoder->images.elements[index]);
