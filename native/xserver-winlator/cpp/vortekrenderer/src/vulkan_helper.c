...
VkResult initVulkanDevice(VkContext* context, VkPhysicalDevice physicalDevice, VkDevice device) {
    if (!context || physicalDevice == VK_NULL_HANDLE || device == VK_NULL_HANDLE) {
    ...
    if (queueFamilyCount == 0 || queueFamilyCount > 64u) {
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    VkQueueFamilyProperties* queueFamilies =
            calloc(queueFamilyCount, sizeof(VkQueueFamilyProperties));
    if (!queueFamilies) return VK_ERROR_OUT_OF_HOST_MEMORY;
...
void disableUnsupportedDeviceFeatures(
    ...
    if (context) context->nullDescriptorEnabled = false;
    ...
    if (context) {
        VkPhysicalDeviceRobustness2FeaturesEXT* robustness2 = findNextVkStructure(
                (void*)createInfo->pNext,
                VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ROBUSTNESS_2_FEATURES_EXT);
        bool robustness2ExtensionEnabled = false;
        for (uint32_t index = 0; index < createInfo->enabledExtensionCount; ++index) {
            ...
            if (extension && strcmp(extension, VK_EXT_ROBUSTNESS_2_EXTENSION_NAME) == 0) {
                robustness2ExtensionEnabled = true;
                break;
            }
        }
        /* This is pending device-publication state.  It becomes authoritative
         * only after vkCreateDevice succeeds and is copied into that device's
         * registry entry; failed or differently-featured devices cannot make
         * descriptor nullability global to the context. */
        context->nullDescriptorEnabled = robustness2ExtensionEnabled && robustness2 &&
...
void destroyVkObject(VkObjectType type, VkDevice device, void* handle) {
    ...
    switch (type) {
        ...
        case VK_OBJECT_TYPE_SAMPLER_YCBCR_CONVERSION:
            vulkanWrapper.vkDestroySamplerYcbcrConversion(device, handle, NULL);
            break;
        default:
            break;
    }
