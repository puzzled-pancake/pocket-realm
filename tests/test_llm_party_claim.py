...
case; this file pins the runtime half that lives in the overlay tree:
  ...
  - the SayAction driver payload (PB_SAY_GATE_ANDROID) consumes
    ...
    only (SRC_RAID joins the CLAIM; recording/digest stay
...
def test_claim_helper_declared_and_mutex_guarded():
    ...
    # refuses the second writer (prior-line residue is
    # erased by the generation-scoped check - see the residue test)
    assert "if (TokenOwnsCurrentLine(key, heardItr->second))" in claim
    # The window EXCEEDS every reachable chat-drain stagger
    ...
    # The key is GROUP-FREE end to end (decl, key function,
...
def test_stand_down_marker_owns_the_addressed_line_round6():
    """Addressed-line sibling: an ADDRESSED line stands
    ...
    # the line (prior-line residue is erased, not obeyed)
    ...
    party = gate.split("The roundtable row + the exactly-one responder")[1].split(
...
def test_payload_consumes_pick_and_claim_at_the_top_of_the_party_block():
    gate = android_anchor("PB_SAY_GATE_ANDROID")
    # The refusal-log if shares the same opening
    # prefix as the strategy gate, so the split key is the FULL gate
    # condition (the refusal if reads !replyGateAllowed)
    party = gate.split("The roundtable row + the exactly-one responder")[1].split(
        "if (sPlayerbotAIConfig.llmEnabled > 0 && hardTriggerAllowed"
        " && partyResponderClaimed && replyGateAllowed")[0]
    ...
    # the claim ANDs into the gated flow (the consume folds
...
def test_recording_preserved_for_all_bots_on_the_unaddressed_leg():
    ...
    # The recording/digest legs sit under a SRC_PARTY
    # guard (raid joins the CLAIM only; raid recording stays
    # party-only) - the legs keep their shape at
    # the deeper indent
    ...
    # the armed-ask consumption survives (test_llm_recall
    # pins the same shape) ...
    ...
    # the digest legs sit inside the same party-only guard
...
def test_addressed_whisper_and_say_never_consult_the_claim():
    ...
    # read in the refusal log, ONE read in the flood
    ...
    # else; the refusal log reads it once between them)
    party = gate.split("The roundtable row + the exactly-one responder")[1].split(
        "if (sPlayerbotAIConfig.llmEnabled > 0 && hardTriggerAllowed"
        " && partyResponderClaimed && replyGateAllowed")[0]
    # declaration + clearing + claim assignment + the refusal-log read
    # + the refund read inside the block; the 6th (and only
    ...
    # behavior is byte-identical with the arm off (HardTriggerAllowed
...
def test_addressed_line_resolves_the_pick_to_the_named_bot_round4():
    """An ADDRESSED party/raid line must yield
    ...
    responder, no interactive-budget double-burn). An addressee that
    is no candidate (dead) picks NOBODY:
    ...
    # The pure pick STANDS DOWN when the addressed guid
...
def test_raid_arm_shares_the_exactly_one_claim_round3():
    """SRC_RAID unaddressed lines are admitted by the
    widened HardTriggerAllowed on the cloud lane, so the claim must
    govern them - ONE generation per line, never one per raid bot
    (which would also burn N interactive-
    budget admissions against the speaker's cap)."""
    gate = android_anchor("PB_SAY_GATE_ANDROID")
    party = gate.split("The roundtable row + the exactly-one responder")[1].split(
        "if (sPlayerbotAIConfig.llmEnabled > 0 && hardTriggerAllowed"
        " && partyResponderClaimed && replyGateAllowed")[0]
...
# ---- the timing layer, closed at both ends ----------------------------------

def test_fanout_stamp_covers_the_leave_before_first_drain_hole_round7():
    """Without the fan-out stamp, the drain-time bystander marker only existed
    ...
    interleaving. The stamp sits AFTER the queue push so its
    ...
    # The lane gate ends at the partyReply key
    ...
    # and the stamp rides the fan-out but AFTER the push (program
    # order makes the marker's stamp >= the entry's m_time, so
    ...
    # The first-heard registry: EVERY member's receive
    ...
    # the channel/speaker gate must CONTAIN both
    # writes (a presence-only pin would survive a de-nested matcher
    # mutant) - the verbatim nested shape is pinned
    nested = (
...
def test_first_heard_registry_gates_the_claim_grant_round10():
    """The mid-drain clock divergence pin: the drain
    ...
    bystander claimed beside the original winner. The exactly-one
    ...
    # (the gate runs BEFORE the ownership check - the residue
...
def test_stand_down_marker_carries_the_same_freshness_gate_round11():
    """TryStandDownPartyLine's second caller pin: the
    ...
    ungated stamp itself prices at >= 2 s - could grant beside the
    original winner. The marker therefore carries the claim's own
    ...
    # Pin the gate block VERBATIM so a whole-
...
def test_residue_token_from_a_prior_line_is_erased_round12():
    """The cross-line residue pin: the claim key is
    ...
    a bystander claim can grant beside it (every
    ingredient an ordinary player action). The ownership check is
    ...
    # The consult is pinned VERBATIM with its own
    # return - a mutant voiding the consult's BODY (call kept,
    # `return false;` gone) survives the substring-only pins above
    ...
    # and the claim's gate is verbatim-pinned too (the inversion class)
    assert ("    if (heardItr == heard.end() ||\n"
            "        now - heardItr->second >=\n"
            "            PARTY_CLAIM_WINDOW_SECONDS - "
            "PARTY_CLAIM_FANOUT_STRADDLE_SECONDS)\n"
...
def test_generation_moved_past_drops_old_line_stragglers_round13():
    """The old-line straggler pin: the generation
    scoping makes both freshness gates and the residue
    ...
    re-registered fresh by a verbatim repeat past the window) would pass
    them all - the gates measure its age against the NEW firstHeard,
    and TokenOwnsCurrentLine erases the old line's still-live winner
    token as residue - the straggler would claim and dispatch a second
    generation for the OLD line while the repeat's own responder is
    refused beside its token. The drain's TTL gate drops such
    ...
    # the dispatch hand-off - nothing that reaches ChatReplyDo can
...
def test_drain_ttl_drops_window_expired_party_lines_round7():
    """The drain-stagger pin: the stagger is ADDITIVE (a master's
    repeated 'wait' adds up to 20 s per invocation; teleport/cast
    chains stack), so no fixed claim window exceeds every reachable
    first drain - a deferred bot could re-claim beside the original
    winner after the window pruned the claim. The line itself expires
    with the window: a queued party/raid line aged past
    PARTY_CLAIM_WINDOW_SECONDS (minus the fan-out straddle
    ...
    # them); >= (not >) is LOAD-BEARING - the prune kills a
    # claim AT stamp+30 (expiresAt <= now), so a strict > oracle leaves
    # one live-line/dead-claim boundary second that re-opens both legs;
    # the invariant is per-LINE, not per-entry - a fan-out
    ...
    drain = android_anchor("PB_AI_DRAIN_STALE_ANDROID")
    # the drop sits between the notBefore hold and the dispatch hand-off
    ...
    # the census counts the TTL drop's own pop+continue; the
    # generation drop (below) adds the third - the notBefore
...
def test_group_switcher_claims_under_the_line_key_round8():
    """The group-switcher pin: a groupId-carrying claim key
    with the group resolved at DRAIN time would let a listener kicked and
    re-invited to another group inside the window compute a FRESH key,
    claim beside the original winner, and deliver a second
    generation to a group that never heard the line. The key is
...
def test_refused_claim_refunds_the_flood_stamp_round7():
    """The flood-refund pin: a claim the picked bot LOST still consumes the
    speaker's 2 s flood slot (identical-text re-send inside the window,
    or the shared party/raid key) - the speaker's next
    DISTINCT line would be denied beside a generation that never happened.
