...
def test_tier_ceremony_gating_and_secret_marker():
    ...
    # ceremony wording never names the mechanic (pin the pure core's
    # string LITERALS - the phrase-selection code may of course read tier)
    import re
    core = CORE.read_text(encoding="utf-8")
    ceremony = core.split("std::string CeremonyUpCargo")[1].split("std::string NicknameOf")[0]
    literals = re.findall(r'"[^"]*"', ceremony)
    assert literals, "the ceremony functions carry wording literals"
    for lit in literals:
