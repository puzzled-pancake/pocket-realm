...
staging window a full staging run silently moved package versions (bash,
libicu, liblzma, readline) by trusting it. stage_mariadb_runtime now refuses
an unpinned index without --refresh; this test pins the four exact
package versions so an accidental refresh cannot ride a commit unnoticed.
Update these pins only alongside a deliberate --refresh staging change.
"""
